...
- **Pulls data** from the [CoinGecko API](https://www.coingecko.com/en/api) — top-10 market data, 7-day hourly price history, and global market stats — and **auto-refreshes every 60 seconds** with a visible countdown and pause control.
- **Visualizes it** with hand-rolled SVG (no chart library, no framework, no build step):
  - stat tiles with 24h deltas and sparklines
...
```
Browser ──60s──▶ CoinGecko API ──fail?──▶ data/snapshot.json (fallback)
                                              ▲
